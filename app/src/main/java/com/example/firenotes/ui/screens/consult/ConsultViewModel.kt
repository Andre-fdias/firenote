...
class ConsultViewModel @Inject constructor(
    ...
    private fun applyFiltersAndSort() {
        ...
        // 2. Specific Filters
        val f = state.filters
        if (f.talao.isNotBlank()) {
            list = list.filter { it.protocolo.contains(f.talao, ignoreCase = true) }
        }
        if (f.natureza != null) {
            list = list.filter { it.natureza == f.natureza }
        }
        if (f.cidade.isNotBlank()) {
            list = list.filter { it.cidade?.contains(f.cidade, ignoreCase = true) == true }
        }
        if (f.bairro.isNotBlank()) {
            list = list.filter { it.bairro?.contains(f.bairro, ignoreCase = true) == true }
        }
        if (f.viatura.isNotBlank()) {
            list = list.filter { o -> o.viaturas.any { it.prefixo.contains(f.viatura, ignoreCase = true) } }
        }
        if (f.militar.isNotBlank()) {
            list = list.filter { o -> o.viaturas.any { v -> v.equipe.any { m -> m.nomeGuerra.contains(f.militar, ignoreCase = true) || m.re.contains(f.militar, ignoreCase = true) } } }
        }
        if (f.placa.isNotBlank()) {
            list = list.filter { o -> o.veiculos.any { it.placa?.contains(f.placa, ignoreCase = true) == true } }
        }
        if (f.nome.isNotBlank()) {
            list = list.filter { o -> o.vitimas.any { it.nome?.contains(f.nome, ignoreCase = true) == true } }
        }
        if (f.hospital.isNotBlank()) {
            list = list.filter { o -> o.vitimas.any { it.hospitalDestino?.contains(f.hospital, ignoreCase = true) == true } }
        }
