...
@Composable
fun OccurrenceNewScreen(
    onNavigateBack: () -> Unit,
    viewModel: OccurrenceFormViewModel = hiltViewModel()
) {
    val uiState by viewModel.uiState.collectAsState()

    // Preencher automaticamente a data e horário atual ao abrir
    LaunchedEffect(key1 = true) {
        viewModel.onEvent(OccurrenceFormEvent.DateChanged(LocalDate.now().toString()))
        val currentTime = LocalTime.now().format(DateTimeFormatter.ofPattern("HH:mm"))
        viewModel.onEvent(OccurrenceFormEvent.DispatchTimeChanged(currentTime))
    }
